...
- Added an independent watchdog as an absolute last resort: a separate timer (not tied to the animation loop at all, and reading the wall clock directly) that forces the results screen to appear if the race has clearly gone on too long or the finish condition was met but somehow missed — this covers a device where the animation loop itself never runs even once, a scenario none of the in-loop error handling above could touch by definition, since it only runs inside frames the loop actually delivers.
- Rebuilt the race loop's architecture from the ground up after the finish screen was still reported as failing with none of the above (individually verified) fixes resolving it. Previously, one `requestAnimationFrame` loop drove both the race simulation (progress, finish detection) and the canvas rendering together — so anything wrong with that one mechanism on a given device put both at risk simultaneously, and the watchdog could only ever be a backup running alongside the same suspect timer. Split them into two fully independent loops: simulation now runs on `setInterval`, and rendering runs separately on `requestAnimationFrame` purely for visuals. Proved this holds even in the worst case tested — `requestAnimationFrame` disabled entirely and the draw function permanently throwing for the whole race — and the results screen still appeared correctly every time, because reaching it no longer depends on rendering succeeding, or even running, at all.
- Cut race length from ~90 seconds to ~20-30 seconds and added a live progress bar to the race screen. A 90-second wait with no obvious sense of how far along you are makes "still running" and "stuck" look identical to a player watching it — this removes that ambiguity outright and makes every test cycle fast.
- Found and fixed a real, confirmed bug from a tester's precise report ("it hit 100% and froze") instead of another guess: high-resolution logging of a live race showed the progress bar reading "100%" for roughly 100ms *before* the race's finish condition actually became true, because the displayed percentage was rounded (`Math.round`) independently of the authoritative finish state. On a loaded device that gap can stretch far longer than 100ms, reading as a genuine freeze. Fixed by never displaying 100% until the race is actually done — the bar now holds at 99% until the exact frame the results screen appears, so there is no window where the screen can look finished while it isn't.
